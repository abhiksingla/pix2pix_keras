    ...
    try:
        ...
        for e in range(nb_epoch):
            ...
            for X_full_batch, X_sketch_batch in data_utils.gen_batch(X_full_train, X_sketch_train, batch_size):
                ...
                losses = [e+1, batch_counter, disc_loss, gen_loss[0], gen_loss[1], gen_loss[2], disc_avg, gen1_avg, gen2_avg, gen3_avg]
            ...
            if (e+1) % 5 == 0:
                gen_weights_path = os.path.join('../../models/%s/gen_weights_epoch%s.h5' % (model_name, e+1))
                generator_model.save_weights(gen_weights_path, overwrite=True)

                disc_weights_path = os.path.join('../../models/%s/disc_weights_epoch%s.h5' % (model_name, e+1))
                discriminator_model.save_weights(disc_weights_path, overwrite=True)

                DCGAN_weights_path = os.path.join('../../models/%s/DCGAN_weights_epoch%s.h5' % (model_name, e+1))
                DCGAN_model.save_weights(DCGAN_weights_path, overwrite=True)

                loss_array = np.asarray(losses_list)
                print (loss_array.shape) # 10 element vector

                loss_path = os.path.join('../../losses/loss_epoch%s.csv' % (e+1))
